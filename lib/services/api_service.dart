...
class ApiService {
  ...
  static const String _baseUrl = 'http://localhost:5000'; // 로컬 테스트용
  ...
  static Future<Map<String, dynamic>?> getRecommendations() async {
    try {
      ...
      if (response.statusCode == 200) {
        print('📡 응답 내용: ${response.body.substring(0, response.body.length > 200 ? 200 : response.body.length)}...');
  ...
  // 📊 추천 데이터 파싱 및 변환
  static Map<String, dynamic> parseRecommendations(Map<String, dynamic> apiData) {
    try {
      final recommendations = apiData['recommendations'] as List<dynamic>? ?? [];

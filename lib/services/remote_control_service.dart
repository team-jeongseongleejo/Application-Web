...
class RemoteControlService {
  ...
  // 📱 기기별 IR 코드 가져오기
  static Future<Map<String, Map<String, dynamic>>> getIRCodes(String deviceId) async {
    try {
  ...
  static Future<bool> sendIRCode({
    ...
    try {
      ...
      final irCodesSnapshot = await _database.child('ir_codes/$deviceId/$command').once();
      ...
      bool success = await _waitForCommandCompletion(commandRef.key!, timeout: 10);
  ...
  // ⏳ 명령 처리 완료 대기 (선택사항)
  static Future<bool> _waitForCommandCompletion(String commandId, {int timeout = 10}) async {
    try {
      ...
      // 명령 상태 모니터링
      subscription = _database.child('ir_commands/$commandId/status').onValue.listen((event) {
  ...
  static Future<void> _updateDeviceStatusFromIR(
    ...
    try {
      ...
      if (statusSnapshot.snapshot.exists) {
        currentStatus = Map<String, dynamic>.from(
          statusSnapshot.snapshot.value as Map
        );
      }
  ...
  static List<MapEntry<String, Map<String, dynamic>>> searchCommands(
    Map<String, Map<String, dynamic>> irCodes, 
    String query
  ) {
